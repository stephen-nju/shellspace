...
# export model_name=/opt/nas/n/mmu/models/Qwen/Qwen3-VL-8B-Instruct
...
# 	--name "${DATE}_Qwen3-VL-8B-Instruct_schedule_r16_ep2_lr1e4_bs4" \
# 	--dataset $schedule_train_ds \
# 	--gradient_accumulation_steps=4 --max_length=4096 --epochs=2 --batch_size 1 --lr=1e-4 --save_strategy=epoch --warmup_ratio 0.05 --save_total_limit=10 \
# 	--eval_dataset $schedule_dev_ds --eval_strategy steps --eval_steps=300

# 纯文本模型训练
export schedule_train_ds=/opt/nas/n/mmu/zhubin/DATA/YOYO_memory/Schedule_train_datasets/20260317_v3/tex_train_datasets_0317.json
export schedule_dev_ds=/opt/nas/n/mmu/zhubin/DATA/YOYO_memory/Schedule_train_datasets/20260317_v3/text_dev_datasets_0317.json

export model_name=/opt/nas/n/mmu/models/Qwen/Qwen3-4B-Instruct-2507
./mstrain.sh --do_train --do_eval --stage sft --finetuning_type lora --lora_rank 16 --lora_alpha 32 --lora_target all-linear --loraplus_lr_ratio 16 \
	--hostfile ${hostfile} \
	--model_name_or_path $model_name \
	--name "${DATE}_Qwen3-4B_Instruct_schedule_r16_ep2_lr1e4_bs4" \
	--dataset $schedule_train_ds \
	--gradient_accumulation_steps=4 --max_length=4096 --epochs=2 --batch_size 1 --lr=1e-4 --save_strategy=epoch --warmup_ratio 0.05 --save_total_limit=10 \
	--eval_dataset $schedule_dev_ds --eval_strategy steps --eval_steps=300
